...
muldiv(type,vtype,lnode)
	...
	int mtype;
		...
		if (!byt) {
			...
			regat[DX]=1;
			if (type != MUL)
				if (vtype[VT] == CINT && (lnode[VT] == CINT
					|| lnode[VT] == CCHAR)) asm_cwd();
		...
		if ((vtype[VT] == CINT) && (lnode[VT] == CINT)) mtype++;
		asm_not(mtype,lnode);
		freev(lnode);
		if (type == MOD) {
			if (byt) {
...
genadd(vtype,lnode)
	...
	if (lnode[VIS] == CONSTV) {
		...
		if (vtype[VIS] == CONSTV) {
			...
			if (vtype[VT] == CCHAR) vtype[VT]=CINT;
			return;
			}
		if (vtype[VIS] == OFFV) {
	...
	if (vtype[VIS] == OFFV) {
		...
		if (vtype[VT] != lnode[VT]) {
			if (vtype[VT] == CCHAR) forceint(vtype);
			else if (lnode[VT] == CCHAR) forceint(lnode);
			}
		forcerm(lnode);
		forcereg(vtype);
		alterv(vtype);
...
gensub(vtype,lnode)
	...
	if (lnode[VT] != vtype[VT])
		if (lnode[VT] == CCHAR) forceint(lnode);
		else if (vtype[VT] == CCHAR) forceint(vtype);
	forcerm(lnode);
	forcereg(vtype);
	alterv(vtype);
	if (is_big) forcees(lnode);
...
gentest(node,vtype)
	...
	if (ntype == PTRTO) {
		...
		if (vtype[VIS] == CONSTV && vtype[VT] == CCHAR) vtype[VT]=CINT;
		force(vtype,AXPAT);
	...
	if (otype == 7) {
		...
		if (vtype[VIS] == CONSTV && !floater(vtype)) {
			...
			if (floater(vtype)) {
				...
				if (is_big) forcees(vtype);
				if (vtype[VIS] == REGV && vtype[VT] != CCHAR)
