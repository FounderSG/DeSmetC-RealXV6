...
/*	A	- leave .a file. do not invoke assembler.
	...
	E	- asks for debug information.
    M   - Intel OMF's.
	S	- asks for dump of the tree.
				...
				case INITVAL:	typ=ctlbyte();
								...
								switch (typ) {
									...
									case CFLOAT:if (vtype[VIS] != CONSTV)
													ierror();
												if (vtype[VT] == CDOUBLE)
													cvt.flt=(&vtype[VVAL])->dbl;
												else if (vtype[VT] == CLONG)
													cvt.flt=(&vtype[VVAL])->lng;
