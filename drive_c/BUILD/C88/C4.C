...
heir20c() {				/*	operator is relational	*/
	int  value;
	char op;
...
heir25c() {				/*	must have constant or left parenthesis	*/
	int  value,node[2];
	char needrp,savestor,savenest;
	unsigned backup,saveat,saveloc,savetype;
	int  temptree[30],ointree,ontree,omfree;
...
/*	look for a parened constant expression	*/
	...
	value=wvalue;
	if (heir != CONSTANT && heir != LCONSTANT) error("must have constant");
	else toknlf();
	return value;
	}
