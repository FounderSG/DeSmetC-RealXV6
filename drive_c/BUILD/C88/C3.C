...
dolabel() {
	int  nodeo[3];
	if (heir == UNDEF) deflab();
	else if (heir != OPERAND) { error("bad label"); return 0; }
...
compound(oldfree,isproc)
	...
	char n,*lastfun,*lowfun,*last_move;
		...
		while (lastfun > oldfree) {
			...
			*wp=0;
			...
			while (bptr <= last_move)
				*oldfree++=*bptr++;
			wp->nchain=hash[32];	/* rebuild function hash chain */
			hash[32]=wp;
			}
		mfree=oldfree;
		}
