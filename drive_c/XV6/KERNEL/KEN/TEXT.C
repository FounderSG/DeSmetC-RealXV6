...
uint a;
{
    ...
    xccdec(rp->p_textp);
    rp->p_flag |= SLOCK;
    if(swapio(rp, a, rp->p_addr, B_WRITE))  /* the hole is not written (swsize) */
        panic("swap error");
    if(ff)
 ...
 * the image is segment-relative position independent and the kernel reaches
 * any page flat, so it is read straight into the shared block (the shortcut
 * V6's own comment suggests) and then written once to the swap image
 * (x_daddr), from which it is reloaded after the in-core copy is released
 * (xccdec) with sharers still swapped out.  SLOCK is held across the whole
 * load, not just the prototype write as in V6: the read target is the
 * shared block itself, so the process must not be swapped out (and the
 * text reloaded by sched) in the middle.
 *
...
uint tsize;
{
        ...
        if(xp->x_iptr == NULL) {
            ...
            if(xp->x_iptr == ip) {
                ...
                u.u_procp->p_textp = xp;    /* V6: attach before out:, so
                                             * sched reloads this text if
                                             * the process waits on swap */
                goto out;
            }
    if((xp=rp) == NULL)
        panic("out of text");
    ...
    u.u_procp->p_flag |= SLOCK;     /* the shared block is the raw read target */
    ...
    if(u.u_error) {
        ...
        u.u_procp->p_taddr = otp != NULL ? otp->x_caddr : 0;
        return;
    }
    /* Write the prototype image to the swap device (V6). */
...
out:
    if(xp->x_ccount == 0) {
        /*
         * Text not in core: read it back in place when core is available
         * (position independent).  SLOCK during the read so this process
         * cannot be picked as a swap victim mid-read -- sched would see
         * x_ccount still 0 at swap-in and reload the text a second time
         * over this one.  With no core, the V6 coroutine: swap self out
         * with SSWAP; sched reloads the text and takes the in-core
         * reference at swap-in.  (The caller's OLD text keeps its in-core
         * reference while we are out -- conservative, and rebalanced by
         * exec's xfree at the commit point.)
         */
        if((ta = malloc(coremap, xp->x_size)) != NULL) {
            ...
            if(save(u.u_ssav) == 0) {
                savu(u.u_procp);    /* the swapped image must carry u_ssav,
                                     * and swsize reads the stored u */
                while((ta = malloc(swapmap,
                        swsize(u.u_procp)*(PAGESIZ/512))) == NULL)
                    sleep(&swapmap, PSWP);
