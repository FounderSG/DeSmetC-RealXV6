 ...
 */
struct buf* breada(adev, blkno, rablkno)
int adev;
int blkno;
int rablkno;
{
 ...
 */
void devstart_rk(bp, devloc, devblk, hbcom)
struct buf *bp;
int devloc;
int devblk;
int hbcom;
 ...
 * swap I/O
 */
int swap(blkno, coreaddr, count, rdflg)
int blkno;
int coreaddr;
int count;
int rdflg;
...
int rw;
{
    ...
    if(base&01 || u.u_count&01 || base >= base+u.u_count)
        goto bad;
    /*
     * Check overlap with text: there is none to check.  The code of a
     * separated I&D program sits in a segment of its own, which is V6's
     * u_sep case -- the one where its text check dropped out as well.
     *
     * Check that the transfer lies in the data segment: the process block
     * less the u-area prefix.  All of it is addressable -- the break and
     * the stack grow toward each other inside a segment that never moves --
     * so unlike V6 there is no unmapped hole in the middle to reject.
     */
    dsize = (uint)(pp->p_size-1)*PAGESIZ + (PAGESIZ-UPREFIX);
    if(base+u.u_count > dsize)
        goto bad;
    /*
     * The disk moves whole 512-byte sectors, so transfer as many full
     * sectors as u_count allows; a sub-sector tail is left in u_count and
     * reported to the caller (through rdwr) as not transferred.
     */
    nsect = u.u_count >> 9;
    if(nsect == 0)
        return;
    ...
    bp->b_dev = dev;
    /*
     * Compute physical address.  Where V6 simulated the segmentation
     * hardware, a real-mode segment base IS the physical address: the
     * user data segment plus the paragraph part of the offset.  The
     * address is absolute, not one taken through the running process's
     * segment registers, because the transfer finishes at interrupt time.
     */
    bp->b_xmem = (void *)(udseg(pp) + (base>>4));
    bp->b_addr = (void *)(base&017);
