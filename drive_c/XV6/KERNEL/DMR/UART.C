 ...
 */
#include <pc.h>             /* KL_BACKEND_UART / KL_SERIAL_TEE */
...
void uart_init()
{
    unsigned char lcr_out, mcr_out, low_divisor, high_divisor;
...
#else
    outportb(COM1_IO_BASE + IER, 0);    /* tee only: writes are polled */
#endif
}
...
void uartintr()
{
    for (;;)
    {
        ...
        case IIR_RECEIVE:
            ...
            klrxintr();
#else
            inportb(COM1_IO_BASE + RBR);  /* tee only: drop serial input */
#endif
            break;
...
char c;
{
    while ((inportb(COM1_IO_BASE + LSR) & 0x20) == 0)
        ;   /* wait for THR empty so boot output is not dropped */
