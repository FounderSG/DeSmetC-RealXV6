 ...
 *
 * Every field being int is what frees this source from the kernel's own
 * headers.  It used to include PARAM.H and PROC.H, because psinfo copied
 * struct proc out verbatim -- and this kernel's struct proc is 29 bytes
 * where the Watcom build's is 28 (p_pri and p_nice are int here, DeSmet's
 * char being unsigned), so the two could never share one ps.
 */
#define STKSIZ	512
