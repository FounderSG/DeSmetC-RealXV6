...
int dosub()
{
	...
	while (c = *rp++) {
		if (c=='&') {
			...
			continue;
		/* compsub() flags an escaped RHS character with 0200 and stores it in
		 * rhsbuf, a char array.  V6 and Watcom read the flag back as a sign
		 * (`c<0`); DeSmet's char is unsigned, so test the bit itself -- which
		 * is what makes \1..\9 back-references work here. */
