 ...
 */
/* uint comes from UNIX.H.  DeSmet C 3.03 has no variadic macros, so the
 * upstream `#define debug(...)` no-op is a real empty function here --
 * K&R passes any argument list through without a prototype. */
debug()
{
}
...
/* time(utime); */
	...
	if(fin < 0) {
		...
		filsys.s_fsize = n;
		filsys.s_isize = mkldiv(0, n, 43+mkldiv(0, n, 1000));
		printf("isize = %d\n", filsys.s_isize);
		if(f_n != 1)
...
struct inode *par;
{
	...
	ino++;
	in.i_number = ino;
	if(mkldiv(0, ino, 16) > filsys.s_isize) {
...
/* 16-bit shims with a dummy high word; renamed off ldiv/lrem so they
 * do not collide with the runtime's real 32-bit ldiv in CTIME.C. */
mkldiv(h, x, y)
{
    return x / y;
}
